  ...
  Object.entries(groupedPosts).forEach(([relativePath, versions]) => {
    if (versions.zh && versions.en) {
      ...
      pairedArticles.push({
        ...
        tagMismatch: zhTags.length !== enTags.length || 
...
pairedArticles.forEach((article, index) => {
  ...
  console.log(`   标签是否匹配: ${article.tagMismatch ? '否' : '是'}`);
  if (article.tagMismatch) {
    console.log(`   >>> 标签数量差异: ${Math.abs(article.zhTags.length - article.enTags.length)}`);
  }
  console.log('');
});