...
function getSortedPosts(lang, allPosts) {
  // 根据语言筛选文章
...
function simulateTagPage() {
  ...
  // 模拟中文标签页面
  const chinesePosts = getSortedPosts('zh', allPosts);
  ...
  // 模拟英文标签页面
  const englishPosts = getSortedPosts('en', allPosts);
  const englishTags = getAllTags(englishPosts);
  const sortedEnglishTags = Object.entries(englishTags)
    .sort(([, a], [, b]) => b - a)
    .map(([tag, count]) => ({ tag, count }));
  
  console.log('=== 模拟标签页面显示 ===');
  console.log('中文标签页面标签数量:', sortedChineseTags.length);
  console.log('英文标签页面标签数量:', sortedEnglishTags.length);
  ...
  // 检查是否有重复的标签名（可能由于特殊字符导致）
  console.log('\n=== 检查特殊字符标签 ===');
  const chineseTagNames = Object.keys(chineseTags);
  const englishTagNames = Object.keys(englishTags);
  
  chineseTagNames.forEach(tag => {
    if (tag !== tag.trim()) {
      console.log('中文标签中有空格:', `"${tag}"`);
    }
  });
  
  englishTagNames.forEach(tag => {
    if (tag !== tag.trim()) {
      console.log('英文标签中有空格:', `"${tag}"`);
    }
  });
  ...
  chineseTagNames.forEach(tag => {
    if (tag.includes('/') || tag.includes('\\') || tag.includes(':')) {
      console.log('中文标签中有特殊字符:', `"${tag}"`);
    }
  });
  
  englishTagNames.forEach(tag => {
    if (tag.includes('/') || tag.includes('\\') || tag.includes(':')) {
      console.log('英文标签中有特殊字符:', `"${tag}"`);
    }
  });
}
